	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (action != null) {
				...
				// END:
				// TODO: 06 logout.do 호출 시 동작할 logout()를 호출하시오.
...
//					url = deleteBoard(request, response);
				...
				// TODO: 17 insertBoardForm.log 호출 시 /board/insertBoard.jsp로 이동하시오.
				else if(action.endsWith("insertBoardForm.log")) { // 로그인햇을때만 하니까 .log (세션아이디를 받아와서 하려고)ㄴㄴ
					url = "/board/insertBoard.jsp";
				}
				// END:

				// TODO: 18 insertBoard.log 호출 시 동작할 insertBoard()를 구현하고 호출하시오.
				else if(action.endsWith("insertBoard.log")) { // 로그인햇을때만 하니까 .log (세션아이디를 받아와서 하려고)ㄴㄴ
	...
	private String listBoard(HttpServletRequest request, HttpServletResponse response) {
		...
		String pageNo = request.getParameter("pageNumber");	//현재 페이지번호
...
//		request.setAttribute("totalNumber", map.get("totalNumber"));
